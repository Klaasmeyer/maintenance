...
**Good:**
```python
def test_retry_operation_retries_failed_operations_3_times():
    attempts = []
...
**Good:**
```python
def retry_operation(action, max_retries: int = 3):
...
def test_submit_form_rejects_empty_email():
    result = submit_form(Form(email=""))
    assert result == Err(FormError.EMAIL_REQUIRED)
```

**Verify RED**
```bash
$ make test
FAIL: AssertionError: Ok(()) != Err(FormError.EMAIL_REQUIRED)
```

**GREEN**
```python
def submit_form(form: Form) -> Result[None, FormError]:
    if not form.email:
        return Err(FormError.EMAIL_REQUIRED)
    return Ok(None)
```
...
When adding mocks or test utilities, review `references/testing-anti-patterns.md` to avoid:
