...
- Reserve for yourself: `pyproject.toml`, configs, any shared files
- Consider worktrees if >2-3 parallel or risk of conflicts

Example:
```
Subagent A: OWN src/processing/parser.py, READ src/models/data.py
Subagent B: OWN src/processing/validator.py, READ src/models/data.py
Orchestrator: src/models/__init__.py (merge exports after)
```
...
|------|--------|-------|---------|
| n7vy.2 | success | parser.py, validator.py | Added input validation |
| 1s08.2 | success | renderer.py | Split into sub-renderers |
```
...
2. Run quality gates ONCE: `make test`
