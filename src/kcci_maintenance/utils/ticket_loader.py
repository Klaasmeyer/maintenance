...
class TicketLoader:
    ...
    def prepare_tickets(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        def clean_value(value, default=''):
            """Convert pandas NaN to default value."""
            if pd.isna(value):
                return default
            return value
        ...
        for _, row in df.iterrows():
            ticket = {
                ...
                'county': clean_value(row.get('county', row.get('County', ''))),
                'city': clean_value(row.get('city', row.get('City', ''))),
                'street': clean_value(row.get('street', row.get('Street', ''))),
                'intersection': clean_value(row.get('intersection', row.get('Intersection', ''))),
                'ticket_type': clean_value(row.get('ticket_type', row.get('Ticket Type'))),
                'duration': clean_value(row.get('duration', row.get('Duration'))),
                'work_type': clean_value(row.get('work_type', row.get('Nature of Work'))),
                'excavator': clean_value(row.get('excavator', row.get('Excavator'))),
            }

            # Add source metadata if available
            if '_source_file' in df.columns:
                ticket['_source_file'] = clean_value(row.get('_source_file', ''))
            if '_source_county' in df.columns:
                ticket['_source_county'] = clean_value(row.get('_source_county', ''))
            if '_source_year' in df.columns:
                ticket['_source_year'] = clean_value(row.get('_source_year', ''))
